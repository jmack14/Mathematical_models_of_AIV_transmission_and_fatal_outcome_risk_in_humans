...
make_proxy_df <- function(res){
  df <- res$sort_output %>%
    mutate(proxy = deaths / meannh * 10000)
...
proxy_1 <- make_proxy_df(res_1)
proxy_2 <- make_proxy_df(res_2)
proxy_3 <- make_proxy_df(res_3)

# ----------------------------
# Quantiles 
# ----------------------------

get_quarts <- function(proxy_df){
  probs <- c(0.025, 0.25, 0.5, 0.75, 0.975)
  idx <- sapply(probs, function(p)
    min(which(proxy_df$cum.prob >= p))
  )
  
  data.frame(
    cprob = proxy_df$cum.prob[idx],
    proxy   = proxy_df$proxy[idx]
  )
}

q1 <- get_quarts(proxy_1)
q2 <- get_quarts(proxy_2)
q3 <- get_quarts(proxy_3)

# ============================================================
# Plot severity proxies: Fig2B
# ============================================================

Fig2B <- ggplot() +
  geom_line(data = proxy_1, aes(x = proxy, y = cum.prob, colour = "38"), size = 0.9) +
  geom_line(data = proxy_2, aes(x = proxy, y = cum.prob, colour = "53.5"), size = 0.9) +
  geom_line(data = proxy_3, aes(x = proxy, y = cum.prob, colour = "18.8"), size = 0.9) +
  
  geom_point(data = q1, aes(x = proxy, y = cprob), size = 1.5) +
  geom_point(data = q2, aes(x = proxy, y = cprob), size = 1.5) +
  geom_point(data = q3, aes(x = proxy, y = cprob), size = 1.5) +
  
  geom_text_repel(data = q1, aes(x = proxy, y = cprob, label = round(proxy,1)),
                  size = 3.5, hjust = 0, vjust = 1) +
  geom_text_repel(data = q2, aes(x = proxy, y = cprob, label = round(proxy,1)),
                  size = 3.5, hjust = 0, vjust = 1) +
  geom_text_repel(data = q3, aes(x = proxy, y = cprob, label = round(proxy,1)),
                  size = 3.5, hjust = 1, vjust = 0) +
...
# ============================================================
# Plot severity comparisons: Fig3
# ============================================================
...
Fig3 <- ggplot(severity_data, aes(x = estimate, y = Virus)) +
  geom_errorbarh(aes(
